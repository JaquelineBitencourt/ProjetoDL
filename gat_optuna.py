...
class GATLayer(nn.Module):
    def __init__(self, in_dim, out_dim, heads=4, concat=True, negative_slope=0.2, dropout=0.3):
...
class GATModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, dropout, 
                 mlp_hidden_dim, mlp_layers, global_dim, heads=4, concat=True, negative_slope=0.2):
        super().__init__()
...
def run_nested_cv(dataset, n_outer_splits=5, n_inner_splits=4, n_trials=20, timeout=None, n_jobs=1, output_dir=None):
    ...
    if output_dir is None:
        output_dir = os.path.join(os.getenv('HOME'), 'gat_optuna_results')
    ...
    for outer_fold, (train_val_idx, test_idx) in enumerate(outer_cv.split(dataset, labels)):
        ...
        study_name = f"gat_optimization_fold_{outer_fold}"
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_optimization_history.png'))
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_param_importances.png'))
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_parallel_coordinate.png'))
            plt.close()
            ...
            if top_params:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_slice_plot.png'))
                plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(os.getenv('HOME'), f'optuna_fold_{outer_fold+1}_results.png'))
    ...
    avg_params["model"]["concat"] = (sum(concat_values) > len(concat_values) / 2)
    
    # Round integer parameters
    for param in ["hidden_dim", "num_layers", "mlp_hidden_dim", "mlp_layers", "heads"]:
        avg_params["model"][param] = int(round(avg_params["model"][param]))
    
    # Save overall results
    with open(os.path.join(output_dir, 'gat_nested_cv_results.txt'), 'w') as f:
    ...
    with open(os.path.join(output_dir, 'gat_nested_cv_results.json'), 'w') as f:
...
def final_model_training(dataset, best_params, output_dir=None):
    ...
    """
    if output_dir is None:
        output_dir = os.path.join(os.getenv('HOME'), 'gat_model_output')
        os.makedirs(output_dir, exist_ok=True)
    ...
    # Save model
    model_path = os.path.join(output_dir, 'final_gat_model.pt')
...
if __name__ == '__main__':
    ...
    output_dir = os.path.join(os.getenv('HOME'), f'gat_optuna_results_{timestamp}')
    os.makedirs(output_dir, exist_ok=True)
    ...
    print(f"Best model saved to: {os.path.join(output_dir, 'final_gat_model.pt')}")
