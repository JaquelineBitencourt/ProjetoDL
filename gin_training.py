...
from torch_geometric.nn import BatchNorm, GINConv, global_mean_pool, global_add_pool, MLP
...
# Hiperparâmetros do modelo - adaptados para GIN
MODEL_HYPERPARAMS = {
    "hidden_dim": 223,
    "num_layers": 4,
    "dropout": 0.1,
    "mlp_hidden_dim": 121,
    "mlp_layers": 1,
    "eps": 0.2,  # Parameter for GIN (epsilon)
    "train_eps": False  # Whether to make epsilon trainable
}

OPTIMIZER_HYPERPARAMS = {
    "lr": 0.000703448423240021,
    "weight_decay": 4.456443795178477e-05
}
...
class GINLayer(nn.Module):
    def __init__(self, input_dim, hidden_dim, eps=0.2, train_eps=False):
...
class GINModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, dropout, 
                 mlp_hidden_dim, mlp_layers, global_dim, eps=0.2, train_eps=False):
        super().__init__()
...
def cross_validate(dataset, model_params, optimizer_params, n_splits=5):
    ...
    for fold, (train_idx, test_idx) in enumerate(skf.split(dataset, labels)):
        ...
        for epoch in range(500):
            ...
            with torch.no_grad():
                for batch in test_loader:
                    ...
                    # Ensure consistent shape handling for validation
                    batch_y = batch.y
                    if batch_y.dim() > 2:
                        batch_y = batch_y.squeeze(0)
                    elif batch_y.shape[0] != output.shape[0]:
                        # Handle case where batch_y might be flattened
                        batch_y = batch_y.view(output.shape[0], -1)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(os.getenv('HOME'), f'Fold_{fold+1}_metrics_ginv3.png'))
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(os.getenv('HOME'), f'Fold_{fold+1}_confmat_ginv3.png'))
