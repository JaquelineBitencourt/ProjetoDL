...
class GraphSAGEModel(nn.Module):
    def __init__(self, input_dim, hidden_dim, num_layers, output_dim, dropout, mlp_hidden_dim, mlp_layers, global_dim, concat=True, negative_slope=0.2):
        super().__init__()
...
def check_and_fix_dataset(dataset):
    """
    Check the dataset for inconsistencies and fix them
    
    Args:
        dataset: List of torch_geometric.data.Data objects
        
    Returns:
        List of fixed torch_geometric.data.Data objects
    """
    logger.info(f"Checking dataset with {len(dataset)} samples...")
    
    # Check for consistent dimensions
    node_feature_dims = set()
    global_feature_dims = set()
    label_dims = set()
    
    # Collect all dimensions
    for i, data in enumerate(dataset):
        node_feature_dims.add(data.x.shape[1])
        
        if hasattr(data, 'global_features'):
            if data.global_features.dim() == 0:
                # Scalar, needs to be expanded
                logger.warning(f"Data point {i} has scalar global_features")
                data.global_features = torch.zeros(5, dtype=torch.float)
            
            global_feature_dims.add(data.global_features.shape[0])
        else:
            logger.warning(f"Data point {i} has no global_features")
        
        if data.y.dim() == 1:
            # Single label case, reshape
            data.y = data.y.unsqueeze(0)
        
        label_dims.add(data.y.shape[1])
    
    # Print the dimensions found
    logger.info(f"Node feature dimensions found: {node_feature_dims}")
    logger.info(f"Global feature dimensions found: {global_feature_dims}")
    logger.info(f"Label dimensions found: {label_dims}")
    
    # Fix inconsistencies
    if len(node_feature_dims) > 1:
        logger.warning("Inconsistent node feature dimensions found!")
        max_dim = max(node_feature_dims)
        for i, data in enumerate(dataset):
            if data.x.shape[1] < max_dim:
                padding = torch.zeros(data.x.shape[0], max_dim - data.x.shape[1], dtype=data.x.dtype, device=data.x.device)
                data.x = torch.cat([data.x, padding], dim=1)
    
    if len(global_feature_dims) > 1 or not global_feature_dims:
        logger.warning("Inconsistent global feature dimensions found!")
        # Choose 5 as the standard dimension
        standard_dim = 5
        for i, data in enumerate(dataset):
            if not hasattr(data, 'global_features') or data.global_features.shape[0] != standard_dim:
                data.global_features = torch.zeros(standard_dim, dtype=torch.float)
    
    if len(label_dims) > 1:
        logger.warning("Inconsistent label dimensions found!")
        max_dim = max(label_dims)
        for i, data in enumerate(dataset):
            if data.y.shape[1] < max_dim:
                padding = torch.zeros(data.y.shape[0], max_dim - data.y.shape[1], dtype=data.y.dtype, device=data.y.device)
                data.y = torch.cat([data.y, padding], dim=1)
    
    # One final check to make sure all dims are consistent now
    node_dims = {data.x.shape[1] for data in dataset}
    global_dims = {data.global_features.shape[0] for data in dataset}
    label_dims = {data.y.shape[1] for data in dataset}
    
    logger.info(f"After fixing - Node dims: {node_dims}, Global dims: {global_dims}, Label dims: {label_dims}")
...
def objective(trial, train_dataset, val_dataset):
    ...
    output_dim = y_shape[1] if len(y_shape) > 1 else 1  # Handle single label case
    
    # Initialize model
    input_dim = train_dataset[0].x.shape[1]
    
    # Get the global features dimension with a more robust approach
    global_features = getattr(train_dataset[0], 'global_features', None)
    if global_features is None:
        global_dim = 5  # Default size
    else:
        if global_features.dim() == 0:
            global_dim = 5  # Default for scalar global_features
        else:
            global_dim = global_features.shape[-1] if global_features.dim() > 1 else global_features.shape[0]
    
    # Log dimensions for debugging
    logger.info(f"Model dimensions - input_dim: {input_dim}, global_dim: {global_dim}, output_dim: {output_dim}")
    ...
    for epoch in range(max_epochs):
        ...
        for batch in train_loader:
            ...
            if batch_y.dim() > 2:
                ...
                batch_y = batch_y.view(-1, 1)  # For single-label case
            
            # Ensure output and batch_y have compatible shapes
            if batch_y.shape[0] != output.shape[0]:
...
def run_nested_cv(dataset, n_outer_splits=5, n_inner_splits=4, n_trials=20, timeout=None, n_jobs=1, output_dir=None):
    ...
    if output_dir is None:
        output_dir = os.path.join(os.getenv('HOME'), 'GS_optuna_results')
    ...
    for outer_fold, (train_val_idx, test_idx) in enumerate(outer_cv.split(dataset, labels)):
        ...
        study_name = f"GS_optimization_fold_{outer_fold}"
        ...
        best_model_params = {
            ...
            "concat": study.best_params["concat"],
            "negative_slope": study.best_params["negative_slope"]
        }
        ...
        try:
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_optimizationGS_history.png'))
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_param_GSimportances.png'))
            plt.close()
            ...
            plt.tight_layout()
            plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_parallel_GScoordinate.png'))
            plt.close()
            ...
            if top_params:
                ...
                plt.tight_layout()
                plt.savefig(os.path.join(output_dir, f'fold_{outer_fold+1}_slice_GSplot.png'))
                plt.close()
        ...
        global_dim = getattr(train_val_data[0], 'global_features', torch.zeros(5)).shape[0]
        output_dim = train_val_data[0].y.shape[1] if train_val_data[0].y.dim() > 1 else 1
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(os.getenv('HOME'), f'optuna_fold_{outer_fold+1}_GSresults.png'))
    ...
    for param in results["best_params"][0]["model"]:
        if param != "concat":  # Skip boolean parameters
    ...
    concat_values = [fold["model"]["concat"] for fold in results["best_params"]]
    avg_params["model"]["concat"] = (sum(1 for x in concat_values if x) > len(concat_values) / 2)
    ...
    with open(os.path.join(output_dir, 'GS_nested_cv_results.txt'), 'w') as f:
    ...
    with open(os.path.join(output_dir, 'GS_nested_cv_results.json'), 'w') as f:
...
def final_model_training(dataset, best_params, output_dir=None):
    ...
    """
    if output_dir is None:
        output_dir = os.path.join(os.getenv('HOME'), 'GS_model_output')
        os.makedirs(output_dir, exist_ok=True)
    ...
    global_dim = getattr(dataset[0], 'global_features', torch.zeros(5)).shape[0]
    output_dim = dataset[0].y.shape[1] if dataset[0].y.dim() > 1 else 1
    ...
    # Save model
    model_path = os.path.join(output_dir, 'final_GS_model.pt')
    ...
    # Save as JSON
    import json
    with open(os.path.join(output_dir, 'model_GSinfo.json'), 'w') as f:
...
if __name__ == '__main__':
    ...
    # Create graph dataset with improved smiles_to_graph function
    dataset = []
    ...
    # Add dataset checking and fixing step
    dataset = check_and_fix_dataset(dataset)
    
    # Create output directory for results
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = os.path.join(os.getenv('HOME'), f'GS_optuna_results_{timestamp}')
    os.makedirs(output_dir, exist_ok=True)
    ...
    print(f"Best model saved to: {os.path.join(output_dir, 'final_GS_model.pt')}")
